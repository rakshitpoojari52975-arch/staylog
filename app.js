/* StayLog — Homestay Manager App
   v4: Calendar view, booking conflict check, guest confirmation PDF */
'use strict';
...
// ─── Conflict checker ─────────────────────────────────────────────────────────
// Returns true if [checkIn, checkOut) overlaps any active booking for the same property
function hasConflict(propertyId, checkIn, checkOut, excludeId=null) {
  return state.data.bookings.some(b => {
    ...
    // Overlap: b.checkIn < checkOut AND b.checkOut > checkIn
...
// ─── Badge ────────────────────────────────────────────────────────────────────
const STATUS_META={confirmed:{label:'Confirmed',bg:'#e8f4ef',color:'#1b5e38'},checkedin:{label:'Checked In',bg:'#e8f0fb',color:'#0d47a1'},checkedout:{label:'Checked Out',bg:'#f0f0ee',color:'#5a5a58'},cancelled:{label:'Cancelled',bg:'#fdeaea',color:'#c62828'}};
...
function renderLoginScreen() {
  ...
  wrap.appendChild(div({style:{fontFamily:'Playfair Display',fontSize:32,color:'var(--accent)',marginBottom:6}},'StayLog'));
  wrap.appendChild(div({style:{fontSize:14,color:'var(--muted)',marginBottom:40,textAlign:'center'}},isSetup?'Set a 4-digit PIN to protect your data':'Enter your PIN to continue'));
  const phaseLabel=div({style:{fontSize:14,color:'var(--muted)',marginBottom:20,textAlign:'center',minHeight:20}},isSetup?'Create PIN':'');
  const dotsWrap=div({style:{display:'flex',gap:16,marginBottom:36}});
  function renderDots(n){ dotsWrap.innerHTML=''; for(let i=0;i<PIN_LENGTH;i++) dotsWrap.appendChild(div({style:{width:18,height:18,borderRadius:'50%',border:'2px solid var(--accent)',background:i<n?'var(--accent)':'transparent',transition:'background .15s'}})); }
  renderDots(0);
  ...
  function handleDigit(d) {
    if(phase==='enter') {
      pinEntry+=d; renderDots(pinEntry.length);
      if(pinEntry.length===PIN_LENGTH) { if(pinEntry===state.auth.pin){state.loggedIn=true;render();}else{msgEl.textContent='Incorrect PIN. Try again.';setTimeout(()=>{pinEntry='';renderDots(0);msgEl.textContent='';},700);} }
    } else if(phase==='create') {
      pinEntry+=d; renderDots(pinEntry.length);
      if(pinEntry.length===PIN_LENGTH) setTimeout(()=>{phase='confirm';phaseLabel.textContent='Confirm PIN';renderDots(0);confirmPin='';},200);
    } else {
      confirmPin+=d; renderDots(confirmPin.length);
      if(confirmPin.length===PIN_LENGTH) { if(confirmPin===pinEntry){const a={pin:pinEntry};state.auth=a;state.loggedIn=true;saveAuth(a);render();}else{msgEl.textContent='PINs do not match. Try again.';setTimeout(()=>{pinEntry='';confirmPin='';phase='create';phaseLabel.textContent='Create PIN';renderDots(0);msgEl.textContent='';},800);} }
    }
  }
  function handleBack() {
  ...
  ['1','2','3','4','5','6','7','8','9','','0','⌫'].forEach(k=>{
    ...
    pad.appendChild(btn({style:{padding:'18px',fontSize:k==='⌫'?20:22,fontWeight:500,background:'var(--white)',border:'1px solid var(--border)',borderRadius:14,cursor:'pointer',color:'var(--text)',boxShadow:'var(--shadow-sm)',transition:'all .1s'},onClick:()=>k==='⌫'?handleBack():handleDigit(k)},k));
  });
  wrap.appendChild(phaseLabel); wrap.appendChild(dotsWrap); wrap.appendChild(msgEl); wrap.appendChild(pad);
  if(!isSetup) wrap.appendChild(div({style:{marginTop:32}},btn({style:{background:'none',border:'none',color:'var(--muted)',fontSize:13,cursor:'pointer',textDecoration:'underline'},onClick:()=>{if(confirm('Reset PIN? You will need to create a new one.')){state.auth=null;state.loggedIn=false;saveAuth(null);renderLoginScreen();}}},'Forgot / Reset PIN')));
...
function renderHeader() {
  ...
  const brand=div({},
    ...
    div({style:{display:'flex',alignItems:'center',gap:8,marginTop:2}},
      ...
      btn({title:'Backup',style:{background:'none',border:'none',padding:'2px 4px',cursor:'pointer',color:'var(--muted)'},onClick:downloadBackup},ico('download',{style:{fontSize:15}})),
      btn({title:'Restore',style:{background:'none',border:'none',padding:'2px 4px',cursor:'pointer',color:'var(--muted)'},onClick:restoreBackup},ico('upload',{style:{fontSize:15}})),
      btn({title:'Lock',style:{background:'none',border:'none',padding:'2px 4px',cursor:'pointer',color:'var(--muted)'},onClick:()=>{state.loggedIn=false;render();}},ico('lock',{style:{fontSize:15}}))
    )
  );
...
function downloadBackup() { const b=new Blob([JSON.stringify(state.data,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download=`staylog-backup-${today()}.json`; a.click(); }
function restoreBackup() { const inp=document.createElement('input'); inp.type='file'; inp.accept='.json'; inp.onchange=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(!d.properties||!d.bookings||!d.expenses)throw 0;if(confirm(`Restore ${d.bookings.length} bookings and ${d.expenses.length} expenses?`)){state.data=d;saveData(d);render();}}catch{alert('Invalid backup file.');}};r.readAsText(file);}; inp.click(); }
...
function renderCalendar() {
  ...
  // Build map: date string → array of bookings active on that date
  const dayMap = {};
  ...
  for(let d=1;d<=daysInMonth;d++) {
    const ds=`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    const bookingsOnDay=dayMap[ds]||[];
    const isToday=ds===today();
    const col=((firstDay+d-1)%7);
    const isLastCol=col===6;
    const cell=div({style:{borderRight:isLastCol?'none':'1px solid var(--border-soft)',borderBottom:'1px solid var(--border-soft)',minHeight:52,padding:'4px',cursor:bookingsOnDay.length>0?'pointer':'default',background:isToday?'#f0faf5':'var(--white)',transition:'background .12s'},
      onClick:()=>{ if(bookingsOnDay.length>0) setState({modal:'calDay',editItem:{date:ds,bookings:bookingsOnDay}}); }
    });
    cell.appendChild(div({style:{fontSize:12,fontWeight:isToday?700:400,color:isToday?'var(--accent)':'var(--text)',marginBottom:3,width:22,height:22,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',background:isToday?'var(--accent)':'transparent',color:isToday?'#fff':'var(--text)'}},String(d)));
    // Show up to 2 booking dots/labels
    bookingsOnDay.slice(0,2).forEach(b=>{
    ...
    if(bookingsOnDay.length>2) cell.appendChild(div({style:{fontSize:9,color:'var(--muted)',paddingLeft:2}},`+${bookingsOnDay.length-2} more`));
    grid.appendChild(cell);
  }
  // Fill remaining
  const totalCells=firstDay+daysInMonth;
  const remaining=(7-totalCells%7)%7;
  for(let i=0;i<remaining;i++) grid.appendChild(div({style:{borderBottom:'1px solid var(--border-soft)',minHeight:52}}));
  ...
  const monthBookings = active.filter(b=>{ const d=new Date(b.checkIn+'T00:00:00'); return d.getFullYear()===year&&d.getMonth()===month; }).sort((a,b2)=>new Date(a.checkIn)-new Date(b2.checkIn));
  if(monthBookings.length>0) {
    wrap.appendChild(h('div',{style:{fontFamily:'Playfair Display',fontSize:16,marginBottom:10}},`Bookings this month (${monthBookings.length})`));
    monthBookings.forEach(b=>wrap.appendChild(bookingCard(b)));
  }
...
<head>
<meta charset="UTF-8"/>
<title>Booking Confirmation — ${b.guestName}</title>
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .logo{font-family:'Playfair Display',serif;font-size:32px;color:#2d6a4f;letter-spacing:-0.01em;margin-bottom:6px}
...
<body>
  <div class="header">
    <div class="logo">StayLog</div>
    <div class="tagline">Booking Confirmation</div>
  </div>

  <div class="hero">
    <h1>Welcome, ${b.guestName}! 🏡</h1>
    <p>We're absolutely delighted to have you stay with us. Your booking is confirmed and we're already looking forward to welcoming you. We hope your stay is everything you're hoping for and more!</p>
  ...
  <div class="section">
    ...
    <div class="detail-grid">
      ...
      ${b.source&&b.source!=='Direct'?`<div class="detail-item"><div class="detail-label">Booked via</div><div class="detail-value">${b.source}</div></div>`:''}
    </div>
  </div>

  <div class="warm-note">
    💚 <strong>A warm note from us:</strong> Our home is your home during your stay. Please make yourself comfortable, help yourself to the kitchen, and don't hesitate to reach out if you need anything at all. We genuinely want you to feel at ease and have a wonderful time!
  </div>

  <div class="rules">
    <h3>🏠 A Few House Guidelines</h3>
    <p style="font-size:13px;color:#7a6830;margin-bottom:16px">We've put these together simply to make the stay enjoyable for everyone — we're sure you'll understand!</p>

    <div class="rule">
      <span class="rule-icon">🌙</span>
      <span class="rule-text"><strong>A peaceful evening for all:</strong> To keep things quiet and restful for the neighbourhood, we kindly ask that you plan to return to the house by <strong>10 PM</strong> each evening. Sweet dreams and good rest for all!</span>
    </div>
    <div class="rule">
      <span class="rule-icon">🕯️</span>
      <span class="rule-text"><strong>Keep it cozy:</strong> This home is a sanctuary, so we kindly ask that you keep the vibe low-key — no parties or large gatherings, please. Just good company and good vibes!</span>
    </div>
    <div class="rule">
      <span class="rule-icon">🌿</span>
      <span class="rule-text"><strong>Fresh air policy:</strong> For the comfort of all guests, the <strong>entire property is non-smoking</strong>. We appreciate your understanding — it keeps the air fresh and the home welcoming for everyone.</span>
    </div>
    <div class="rule">
      <span class="rule-icon">💡</span>
      <span class="rule-text"><strong>A little energy saving:</strong> When you head out, please help us save a bit of energy by turning off the <strong>lights, A/C, and fans</strong>. Small steps, big impact — thank you!</span>
    </div>
    <div class="rule">
      <span class="rule-icon">🍽️</span>
      <span class="rule-text"><strong>Kitchen courtesy:</strong> The kitchen is all yours! We just request that any <strong>utensils used be washed</strong> after use, so the next guest (or yourself the next morning!) comes back to a clean space.</span>
    </div>
    <div class="rule">
      <span class="rule-icon">👟</span>
      <span class="rule-text"><strong>Shoes at the door:</strong> We'd love it if you could <strong>leave your footwear outside</strong> the house. It keeps our floors clean and adds that lovely homey touch!</span>
    </div>
  </div>

  <div class="footer">
    <p>We hope you have a truly memorable and relaxing stay. 🌸</p>
    <p style="margin-top:6px">If you need anything, please don't hesitate to get in touch.</p>
    <p style="margin-top:12px;font-weight:600;color:#2d6a4f">Generated by StayLog · ${new Date().toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}</p>
  </div>
</body>
</html>`;
  ...
  a.download = `StayLog-Confirmation-${b.guestName.replace(/\s+/g,'-')}-${b.checkIn}.html`;
  a.click();
}
...
// ─── Bookings Tab (list + calendar toggle) ────────────────────────────────────
...
function renderExpenses() {
  ...
  const wrap=div({style:{padding:'14px 12px 100px'}});
  wrap.appendChild(div({style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}},h('div',{style:{fontFamily:'Playfair Display',fontSize:20}},'Expenses'),btn({className:'btn-primary btn-sm',onClick:()=>setState({modal:'addExpense',editItem:null})},ico('plus',{style:{marginRight:4}}),'Add')));
...
function downloadReport(bookings, expenses, periodLabel) {
  ...
  csv+='\nEXPENDITURE DETAILS\nDate,Description,Property,Category,Amount,Notes\n';
  [...expenses].sort((a,b)=>new Date(a.date)-new Date(b.date)).forEach(e=>{ csv+=[esc(e.date),esc(e.description),esc(propName(e.propertyId)),esc(e.category),Number(e.amount||0),esc(e.notes||'')].join(',')+'\n'; });
...
function render() {
  ...
  if(state._loading) {
    app.innerHTML='';
    app.appendChild(div({style:{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'100vh',gap:16}},h('div',{style:{fontFamily:'Playfair Display',fontSize:28,color:'var(--accent)'}},'StayLog'),h('div',{style:{fontSize:13,color:'var(--muted)'}},'Loading your data…')));
    return;
  }
